 ...
 * [pendingTypingFormats] («режим ввода») как есть, БЕЗ повторной синхронизации с контекстом
 * каретки здесь: её уже сделали `onSelectionChanged` при последнем перемещении курсора или
 * явная синхронизация сразу после загрузки заметки (см. `update` в `BibleEditText`). Пересинх
 * на каждом нажатии стирала бы предыдущие переключения той же каретки — текст вокруг неё не
 * меняется между нажатиями, поэтому набор откатывался бы к исходному состоянию и не позволял
 * снять все стили разом (баг: снял курсив — снялся, снял жирный — жирный снялся, но курсив
 * снова стал активным).
 *
