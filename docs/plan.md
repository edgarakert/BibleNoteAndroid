...
- [x] Точность напоминания: окно доставки 15 минут вместо часа — решение владельца продукта
...
- [x] Колонка форматирования в `Note` + автомиграция 3→4
- [x] Кодек диапазонов форматирования (свой формат вместо HTML — `fromHtml` не читает размер шрифта обратно)
- [x] `BibleEditText`: применение и извлечение `StyleSpan` / `RelativeSizeSpan`
- [x] Панель форматирования: жирный, курсив, размер
- [x] Доработка `NoteContentWriter` под форматирование
- [x] Сквозная проверка: старые заметки, ссылки внутри форматирования, поиск, поворот экрана
- [x] Вставка стиха из шторки оформляется как в iOS: курсив + приглушённый цвет, под ним мелкая ссылка (типы `q`/`c` в кодеке)
