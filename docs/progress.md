...
### Ждёт решения владельца продукта

- **Точность напоминания.** `setAndAllowWhileIdle`, выбранный в плане, — неточный будильник: `dumpsys alarm` показывает `window=+1h`, то есть уведомление на 09:00 может прийти в 09:50. План исходил из «точность до минуты не нужна», но окно в час заметно больше. Варианты: оставить как есть; `setWindow` с окном ~10–15 минут — точнее, но в режиме Doze откладывается; точный будильник — требует `SCHEDULE_EXACT_ALARM`/`USE_EXACT_ALARM`, которые Google Play разрешает только будильникам и календарям.
