...
### 2026-09-19 — выключенный стиль не действовал на следующий набранный текст

Пользователь: набрал жирное слово, пробел, выключил жирность, набрал слово — оно осталось жирным. **Причина:** спаны создаются с `SPAN_EXCLUSIVE_INCLUSIVE`, поэтому текст, набранный вплотную к концу жирного диапазона, автоматически входит в него; `afterTextChanged` умел только добавлять включённые стили и не снимал унаследованные. Дополнительно `onSelectionChanged` при наборе пересобирал режим ввода из окружения (символ слева ещё жирный) и затирал выбор пользователя. **Исправлено:** в `afterTextChanged` для вставленного диапазона включённые стили добавляются, выключенные снимаются (`removeStyle`); флаг `isInserting` не даёт `onSelectionChanged` пересинхронизировать режим ввода во время вставки. Проверено на эмуляторе: «aaa» жирное → пробел → жирность выкл → «bbb» обычное → жирность вкл → «ccc» жирное.

### 2026-09-19 — стиль красил всё слово, а не только новый символ

Пользователь: набрал слово без форматирования, не ставя пробел включил жирность и напечатал символ — жирным стало всё слово. **Причина:** клавиатура ведёт слово как composing text и при каждой букве заменяет его целиком, поэтому `onTextChanged` сообщал диапазон всего слова, и режим ввода применялся ко всему диапазону (после предыдущего фикса ещё и снимался/ставился на весь диапазон). **Исправлено:** в `beforeTextChanged` снимается копия заменяемого текста и стиль каждого его символа; в `afterTextChanged` символы общего префикса/суффикса со старым текстом получают обратно свои прежние стили, а режим ввода применяется только к реально новым. Проверено на эмуляторе с экранной клавиатурой: «hello» обычное → жирность вкл → «x» жирное, «hello» не тронуто; «xy» остаются жирными при переключении на курсив для «z».

### 2026-09-19 — оформление вставленного стиха как в iOS

Пользователь: в iOS вставленный стих отличается стилем (курсив, приглушённый серый, под ним мелкая жёлтая ссылка), в Android вставлялся плоский текст. В локальном репо iOS такой вставки нет — сделано по описанию. **Реализация:** в `FormatType` добавлены `QUOTE` (кодек `q`) и `CAPTION` (`c`) — без кнопок тулбара, их ставит только вставка. `VerseSnippetBuilder.quote` собирает «стихи\nссылка» с диапазонами ITALIC+QUOTE на стихи и CAPTION на ссылку; ссылка строится из полного названия книги (`uiState.bookName`), поэтому парсер подсвечивает её янтарём и делает кликабельной. `PendingEdit.formatting` — стиль вставки: после `replace` со вставленного диапазона снимаются растянувшиеся соседние спаны (`SPAN_EXCLUSIVE_INCLUSIVE`) и накладываются свои. Спаны: `VerseQuoteSpan` — маркер, цвет `onSurfaceVariant` рисует `applyHighlighting` между чернилами и янтарём (иначе сплошной чернильный спан перекрывал бы цвет); `CaptionSizeSpan` — свой `MetricAffectingSpan` ×0.85, не `RelativeSizeSpan`, чтобы кнопка «размер» не принимала подпись за крупный текст. Набор внутри стиха наследует QUOTE (как typingAttributes в iOS), после подписи — обычный текст. Тесты: `VerseSnippetBuilderTest` (структура цитаты, ссылка-подпись парсится), `NoteFormattingCodecTest` (круг `q`/`c`). Проверено на эмуляторе: вставка Иоанна 3:16-17 под курсивной строкой — стихи серым курсивом, подпись мелкая янтарная; после выхода и повторного открытия оформление на месте; набранное под подписью — обычный текст. Ревью субагентом (kotlin-reviewer): найдено, что символ, вписанный в середину подписи, выходил обычного размера и разрывал её — `syncFromContext` теперь наследует CAPTION, если каретка внутри строки подписи (не в её конце); проверено на эмуляторе.

### 2026-09-19 — тап по ссылке в редакторе в фокусе переносил курсор вместо открытия стиха

Пользователь: при активном редакторе тап по ссылке на стих ставил каретку на ссылку, а шторка не открывалась. **Причины:** (1) проверка попадания в ссылку не учитывала внутреннюю прокрутку `EditText` (`scrollX/scrollY`) — когда появляется клавиатура, поле сжимается и прокручивается, и координаты тапа мимо ссылки пересчитывались в другую строку; (2) обработчик смотрел только `ACTION_UP`, а DOWN/MOVE целиком доставались `EditText`, который в фокусе ставит каретку и начинает её перетаскивание. **Исправлено:** проверка попадания вынесена в `EditText.bibleSpanAt` с учётом прокрутки; ссылка запоминается на `ACTION_DOWN`, и если это короткий тап (без сдвига больше touch slop, короче long-press) по той же ссылке, на `ACTION_UP` `EditText` получает `ACTION_CANCEL`, каретка возвращается на позицию до касания, открывается шторка. DOWN/MOVE по-прежнему отдаются `EditText`, поэтому долгое нажатие на ссылку выделяет текст, как раньше. Проверено на эмуляторе: редактор в фокусе с клавиатурой, текст прокручен — тап по «John 3:16-17» открывает шторку. Ревью субагентом (kotlin-reviewer): сравнение спана DOWN/UP по identity ломалось, если `applyHighlighting` (debounce 400 мс после ввода) пересоздавал `BibleClickSpan` посреди касания — теперь на DOWN запоминается только факт попадания в ссылку, а спан на UP ищется заново; `ACTION_POINTER_DOWN` сбрасывает тап.

### 2026-09-19 — ссылка открывала шторку на чужих стихах

Пользователь: в заметке «Бытие 1:3», ниже через строку «Бытие 1:1» — тап по 1:1 открывает стих 3. Со вставкой стиха не связано, баг старый. **Причина:** `BibleVerseSheetViewModel` получался через `koinViewModel(key = reference.id)` в хранилище экрана редактора и жил, пока открыта заметка. Шторка правит ссылку в тексте: «Бытие 1:1» с выбранным в шторке стихом 3 становилась «Бытие 1:3», а ViewModel с ключом `1/1/1/0` оставалась с выбором {3}. Новая «Бытие 1:1» давала тот же ключ и получала эту ViewModel. Воспроизведено на эмуляторе. **Исправлено:** у `BibleVerseSheet` параметр `sessionKey` (по умолчанию `reference.id` — для шторки молитв, где ссылка не правится); редактор передаёт `"${ref.id}#$sheetSession"`, где `sheetSession` — `rememberSaveable`-счётчик, растущий на каждый тап по ссылке. Поворот экрана ключ не меняет, выбор в открытой шторке сохраняется (проверено). Проверено на эмуляторе: 1:1 → в шторке выбран 3 вместо 1 → новая «Gen 1:1» ниже открывается на «Бытие 1:1».

### 2026-09-22 — три бага с тестового APK (скриншот)

Пользователь протестировал сборку на устройстве, прислал фото редактора с тремя проблемами.

- **Вставка текста стиха дублировала ссылку.** Кнопка «Вставить текст стиха» в шторке всегда срабатывает под ссылкой, которая уже есть в тексте заметки (шторка открывается тапом по ней). `onInsertVerses` в `NoteEditorScreen.kt` собирал вставку через `VerseSnippetBuilder.quote(verses, reference)` — стихи курсивом плюс полная ссылка мелким шрифтом под ними. На фото это дало «Быт 1:1-4» (сама ссылка в тексте заметки) сразу над вставленными стихами и «Бытие 1:1-4» (подпись из `quote`) под ними — одна и та же ссылка дважды. Собственный докстринг `versesBody` уже описывал ровно этот сценарий («вставка под ссылку, которая в тексте уже есть — дублировать её незачем»), но вызывающий код использовал не ту функцию. **Исправлено:** добавлена `VerseSnippetBuilder.quoteBody` — тот же италик/приглушённый стиль стихов, без строки ссылки; `onInsertVerses` переключён на неё. `quote()` с подписью не удалён — тестами покрыт и может понадобиться для других сценариев (шторка молитв не редактируема, туда путь и не ведёт). Добавлены юнит-тесты `quoteBody`.
- **Тап по ссылке иногда всё ещё переносил курсор.** Механизм DOWN/MOVE/UP из фикса 2026-09-19 запоминает на `ACTION_DOWN`, что палец лёг на ссылку, и сбрасывает это, если `ACTION_MOVE` уходит дальше touch slop — но сам повторный поиск спана на `ACTION_UP` смотрел по координатам ОТПУСКАНИЯ (`event.x/y`), а не исходного касания. Палец мог сместиться на несколько пикселей в пределах допустимого slop (обычный дрожащий тап), и это смещение уже выходило за узкий хитбокс короткой ссылки («Быт 1:1-4») в точной проверке `bibleSpanAt` (`x in spanStartX..spanEndX`) — жест целиком откатывался к обычной установке каретки. **Исправлено:** повторный поиск спана на UP теперь идёт по `downX/downY` (координатам касания), а не по координатам события UP; сам факт «короткий тап, без сдвига дальше slop» по-прежнему решает MOVE-хендлер, так что перетаскивание/долгое нажатие не затронуты. Тест на устройстве не автоматизирован (нет Robolectric/инструментальных тестов на touch в проекте) — логика проверена трассировкой состояний DOWN→MOVE→UP вручную и ревью-субагентом.
- **Шторка стиха открывалась сразу на весь экран.** `LazyColumn` со списком стихов был на `Modifier.weight(1f, fill = false)` внутри `Column`-контента `ModalBottomSheet`. `fill = false` не помогает: `LazyColumn`, в отличие от обычного `Column`, не схлопывается по контенту и всегда занимает весь выданный ему максимум высоты — поэтому измеренная высота контента шторки равнялась высоте экрана, и «частично развёрнутое» состояние `ModalBottomSheet` совпадало с «полностью развёрнутым». **Исправлено:** `LazyColumn` получил `.fillMaxHeight(0.45f)` — доля от входящего ограничения высоты. Первая версия фикса считала долю от `LocalConfiguration.current.screenHeightDp`; по замечанию ревью-субагента заменено на `fillMaxHeight(fraction)` — он берёт долю от уже переданного шторке ограничения (учитывающего инсеты) и не тянет лишнюю рекомпозицию при смене конфигурации.

Сборка (`compileDebugKotlin`) и весь юнит-тестовый набор (`testDebugUnitTest`) — зелёные. Ревью субагентом (kotlin-reviewer): 0 CRITICAL/HIGH/MEDIUM, один LOW (уточнение fillMaxHeight выше, применено); заодно отмечено, что `VerseSnippetBuilder.quote()` и параметр `reference` у `onInsertVerses` стали неиспользуемыми в продакшен-коде — оставлены (`quote()` покрыт тестами, может понадобиться для других сценариев), заведена как задача на будущее, а не как баг.

#### Вставка стиха заменяет ссылку (2026-09-23)

По просьбе пользователя: «Вставить текст стиха» больше не добавляет стихи абзацем под строкой со ссылкой — ссылка удаляется, и на её месте появляется `VerseSnippetBuilder.quoteBody` (стихи курсивом, приглушённым цветом). `onInsertVerses` в `NoteEditorScreen.kt` шлёт `PendingEdit` на диапазон самой ссылки (`activeRange`, если шторка уже переписала ссылку через `onVersesChanged`, иначе границы `ref`). Сборка и юнит-тесты — зелёные.

Доработка по отзыву:
- **Отдельная строка.** `NoteTextInsertion.lineEndAfter` заменён на `replaceOnOwnLine(content, start, end, block)`: съедает пробелы вокруг ссылки, добавляет `\n` перед блоком, если ссылка не в начале строки, и после — если за ней на строке есть текст или это конец заметки. Возвращает `textOffset`, на который сдвигаются диапазоны форматирования. Покрыт юнит-тестами.
- **Стиль цитаты протекал в текст пользователя.** Две причины: (1) после вставки каретка оставалась где была — часто вплотную к концу цитаты; (2) `syncFromContext` у конца цитаты наследовал `QUOTE` и курсив («продолжение цитаты»). Исправлено: `PendingEdit.moveCursorToEnd` ставит каретку за вставку и синхронизирует режим ввода; `syncFromContext` не наследует `QUOTE` и курсив цитаты, если каретка ровно на конце `VerseQuoteSpan` (внутри цитаты стиль по-прежнему продолжается).
- **Номера стихов в цитате.** Новый `FormatType.VERSE_NUMBER` (код `n` в `NoteFormattingCodec`) — `VerseNumberSpan` (`MetricAffectingSpan`, 0.75 размера); цвет — янтарь ссылок, его ставит `applyHighlighting` поверх приглушённого цвета цитаты (как и сам цвет `QUOTE`). `VerseSnippetBuilder.quoteBody` добавляет диапазоны номеров, считая смещения по списку стихов, а не разбором текста. `syncFromContext` номер не наследует — набранное после номера идёт обычным текстом цитаты. Старые заметки без `n` открываются как раньше. Тесты: `quoteBody` (в т.ч. многозначные номера и дубли), round-trip кодека.
- **Ссылка-подпись под цитатой.** `onInsertVerses` снова использует `VerseSnippetBuilder.quote(verses, reference)`, теперь построенный поверх `quoteBody` (с номерами стихов): под стихами — строка со ссылкой (полное название книги + вставленные стихи) типа `CAPTION` (0.85 размера), янтарная и кликабельная через парсер. Раньше `quote()` дублировал ссылку, стоявшую над вставкой; теперь исходная ссылка заменяется, так что дубля нет.
- **«Убрать текст стиха».** `NoteTextInsertion.removeQuoteAbove(content, runs, refStart, refEnd)` узнаёт подпись вставленной цитаты: ссылку покрывает `CAPTION`, а строка прямо над ней кончается диапазоном `QUOTE` (смежные куски `QUOTE`, разрезанные набором внутри цитаты, склеиваются). Возвращает правку «[начало цитаты, конец подписи) → текст подписи» — с `formatting = emptyList()`, так что ссылка становится обычной. `BibleVerseSheet` получил необязательный `onRemoveVerses`: если он задан, вместо «Вставить текст стиха» показывается «Убрать текст стиха» (`verse_remove_text`, ru/en). `NoteEditorScreen` вычисляет его по живым `content`/`formatting` и `activeRange`. Тесты: круговой «вставили → убрали», разрезанная цитата, обычная ссылка, подпись без цитаты.
